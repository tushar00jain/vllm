...
# BUG-2 workaround: Use individual isend/irecv instead of batch_isend_irecv
@torchcomms_worker_fn_wrapper
def safe_batch_isend_irecv_torchcomms_worker():
    from vllm.distributed.eplb.eplb_communicator import _safe_batch_isend_irecv

    rank = torch.distributed.get_rank()
    coordinator = _create_coordinator(False, "test_safe_batch_p2p_tc")
    ...
    reqs = _safe_batch_isend_irecv(ops)
    for req in reqs:
        req.wait()
...
def test_safe_batch_isend_irecv_torchcomms():
    """BUG-2 workaround: _safe_batch_isend_irecv falls back to individual
    isend/irecv when torchcomms is active, avoiding _BackendWrapper
    coalescing issues."""
    hung, exitcodes = distributed_run_with_timeout(
        safe_batch_isend_irecv_torchcomms_worker,
        2,
        extra_env={"USE_TORCHCOMMS": "True"},
        timeout=60,
    )
    assert not hung, "P2P ops should not hang with _safe_batch_isend_irecv"
