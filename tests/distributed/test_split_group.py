...
def _world_device_backend() -> str:
    ...
    Hardcoding a name here fails with "Backend mismatch for device 'cuda'".
    """
...
def broadcast_object_worker():
    ...
    assert result == obj, f"broadcast_object failed: expected {obj}, got {result}"
...
def send_recv_object_worker():
    ...
    if coordinator.rank_in_group == 0:
        ...
        assert result == obj, f"recv_object failed: expected {obj}, got {result}"
...
def all_reduce_worker():
    ...
    assert torch.all(result == expected).cpu().item(), (
        f"all_reduce failed: expected {expected}, got {result.flatten()[:4].tolist()}"
    )
...
def all_gather_worker():
    ...
    if use_dc:
        ...
        output = torch.empty(4 * world_size, dtype=torch.float32, device=device)
    ...
    assert torch.equal(result, expected), (
        f"all_gather failed: expected {expected.tolist()}, got {result.tolist()}"
    )
...
def reduce_scatter_worker():
    ...
    world_size = coordinator.world_size
    chunk_size = 4
    tensor = torch.ones(world_size * chunk_size, dtype=torch.float32, device=device)
    if use_dc:
    ...
    assert torch.all(result == expected_val).cpu().item(), (
        f"reduce_scatter failed: expected {expected_val}, got {result.tolist()}"
    )
...
def send_recv_gpu_tensor_worker():
    ...
    if coordinator.rank_in_group == 0:
        ...
        if use_dc:
            result = coordinator.recv(size=torch.Size([16]), dtype=torch.float32, src=0)
        else:
        ...
        torch.accelerator.synchronize()
        expected = torch.full((16,), 42.0, dtype=torch.float32, device=device)
        assert torch.equal(result, expected), (
...
def broadcast_tensor_dict_worker():
    ...
    assert result["scalar"] == 42, (
        f"broadcast_tensor_dict scalar mismatch: expected 42, got {result['scalar']}"
    )
    assert result["text"] == "hello", (
        f"broadcast_tensor_dict text mismatch: expected 'hello', got {result['text']}"
    )
...
def send_recv_tensor_dict_worker():
    ...
    if coordinator.rank_in_group == 0:
        tensor_dict = {
            "gpu_tensor": torch.full((8,), 99.0, dtype=torch.float32, device=device),
            "scalar": 42,
        }
        ...
        assert result is not None
        expected_tensor = torch.full((8,), 99.0, dtype=torch.float32, device=device)
        assert torch.equal(result["gpu_tensor"], expected_tensor), (
        ...
        assert result["scalar"] == 42, (
            f"recv_tensor_dict scalar mismatch: expected 42, got {result['scalar']}"
        )
...
def distributed_run_with_timeout(fn, world_size, extra_env=None, timeout=60):
...
@pytest.mark.skip(
    reason="torchcomms shim removed; vLLM uses split_group / nccl-lazy directly."
)
@pytest.mark.skipif(
    torch.accelerator.device_count() < 2,
    reason="Need at least 2 GPUs to run the test.",
...
def all_gather_into_tensor_torchcomms_worker():
    ...
    tensor = torch.full((16,), float(rank), dtype=torch.float32, device=device)
    output = torch.empty(
...
def flashinfer_backend_resolve_torchcomms_worker():
    ...
    assert backend == "mnnvl", f"Expected mnnvl backend with torchcomms, got {backend}"
